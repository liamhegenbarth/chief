...
class Chief {
	...
	getCommand()
	{
		...
		{
			return _.message('help!');
		}
		// wrong command, return list of help commands
		else if ( this.args.length > 0 )
		{
			return _.message('nothing found! Heres a list');
		}
		// just run
		else
		{
	...
	// set flow based on provided commands
	handleCommand()
	{
	...
	buildServer()
	{
		...
		this.app.use(this.express.static(`${this.settings.public}`));
		// this.app.use(this.express.static(`${this.settings.compiled}`));
		this.app.use(this.express.Router());
	...
	setMiddleware()
	{
		...
		if ( this.settings.middleware )
		{
			...
			for ( let m = 0; m < this.settings.middleware.length; m++ )
			{
			...
			( err )
				...
				? _.read.file(`${this.settings.content}/${req.url}/${this.settings.index}.md`, (err, file) => {
					...
					( err )
						...
						? this.renderDev(err, req, res, file)
	...
	getRouteProd( req, res )
	{
		...
		// !!!  res.send()  !!! //
		// !!!  res.sendFile() this might be the one !!! //
		// !!!  res.set() for content type headers  !!! //
		// !!!  res.status() for proper codes  !!! //
		// !!!  res.type() for MIME type? !!! //
		...
		const extension = ( this.settings.pretty ) ? '.html' : '';

		const fileName = `${this.settings.compiled}/${req.url}${extension}`;
	...
	compile()
	{
		...
		// check content and compile directories exist
    ...
    checkContent( matrix )
    {
    	...
    	try
    	{
    		_.access.dir(this.settings.content);
    	}
    	catch(err)
    	{
    		_.error(err, this.settings);
    	...
    	this.checkCompiled(matrix);

    }


    checkCompiled( matrix )
    {

    	try
    	{
    		_.access.dir(this.settings.compiled);
    	}
    	catch(err)
    	{
    		_.error(err, this.settings);
    ...
    eraseDirectories( matrix )
    {
        ...
        _.stopwatch.stop('compile');

        _.message(`Chief ran compile. Took ${ _.stopwatch.get('compile') }. Chief did good`);
    ...
    {
    	return _.message(`Chief running with wolves. You look, port ${this.settings.port}`);
    }
